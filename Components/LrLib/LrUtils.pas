...
procedure FindComponentByClass(out outComponent; inClass: TClass;
	inOwner: TComponent);
...
function MakeFilter(const inDescription, inExtensions: string): string;
...
function MakeFilter(const inDescription, inExtensions: string): string;
begin
	Result := inDescription + '(' + inExtensions + ')|' + inExtensions + '|';
end;
