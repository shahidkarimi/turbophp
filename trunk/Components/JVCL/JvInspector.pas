...
type
  ...
  public
	...
	// TList of PPropInfos with machinery to compare and merge properties
...
//      FListBox := TJvPopupListBox.Create(Inspector);
    ...
    TCustomEditAccessProtected(EditCtrl).OnMouseUp := EditMouseUp;
		//TCustomEditAccessProtected(EditCtrl).OnChange := EditChange; //sjm2005
		TCustomEditAccessProtected(EditCtrl).OnChange := nil; //sjm2005
...
begin
  ...
  begin
    ...
    try
			DeleteMembers;
			TJvInspectorPropData.New(Self,
				TJvInspectorPropData(Data).CreateInstanceList);
...
type
	TJvInspectorPropDataFactory = class
...
//=== { TJvInspectorPropDataFactory } =========================

class procedure TJvInspectorPropDataFactory.ManufactureItems(
	const AParent: TJvCustomInspectorItem; const Instances: TList);
begin
	with TJvInspectorPropDataFactory.Create do
	try
...
class function TJvInspectorPropDataFactory.GetJvPropList(
...
procedure TJvInspectorPropDataFactory.SetObjects(inList: TList);
var
	i: Integer;
	p: TJvPropList;
begin
...
procedure TJvInspectorPropDataFactory.MergeProps;
var
	c, i, j: Integer;
	p: TJvPropList;
	match: Boolean;
...
function TJvInspectorPropDataFactory.CreateItem(
...
begin
	...
	if pprop = nil then
		raise EJvInspectorData.CreateRes(@RsEJvAssertPropInfo);
	//
	data := TJvInspectorPropData.CreatePrim(pprop.Name, pprop.PropType^);
	//
	...
	data.Prop := pprop;
	//
	data := TJvInspectorPropData(TJvInspectorPropData.RegisterInstance(data));
	//
	if data <> nil then
	begin
...
procedure TJvInspectorPropDataFactory.CreateItems(
	const AParent: TJvCustomInspectorItem);
var
	i: Integer;
	p: TJvPropList;
...
function TJvInspectorPropData.CreateInstanceList: TList;
var
	i: Integer;
begin
	Result := TList.Create;
...
class function TJvInspectorPropData.New(
	const AParent: TJvCustomInspectorItem; const Instances: TList;
	const TypeKinds: TTypeKinds): TJvInspectorItemInstances;
begin
	TJvInspectorPropDataFactory.ManufactureItems(AParent, Instances);
	Result := nil;
	Instances.Free;
end;
...
begin
	if FItems.IndexOf(RegItem) < 0 then
	  FItems.Add(RegItem);
end;
