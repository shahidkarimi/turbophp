...
uses
	...
	CustomInspector, LrDynamicProperties;

type
	TDynamicProperty = class(TCustomProperty)
	private
		FComponent: TPersistent;
		FItem: TLrDynamicPropertyItem;
	protected
		procedure SetItem(const Value: TLrDynamicPropertyItem);
	public
		...
		property Item: TLrDynamicPropertyItem read FItem write SetItem;
	end;
	//
	...
	private
		...
		FProperties: TLrDynamicProperties;
	public
		function GetAttributes: TPropertyAttributes; override;
		function GetName: string; override;
		function GetValue: string; override;
		...
		procedure SetValue(const Value: string); override;
		property Component: TPersistent read FComponent write FComponent;
		property Properties: TLrDynamicProperties read FProperties write FProperties;
	...
	protected
		function CreateProperty(inType: TLrDynamicPropertyType): TDynamicProperty;
		function GetProperties(inComponent: TPersistent): TLrDynamicProperties;
		procedure AddNewProp(inComponent: TPersistent;
			inProperties: TLrDynamicProperties; Proc: TGetPropEditProc);
...
procedure TDynamicProperty.SetItem(const Value: TLrDynamicPropertyItem);
begin
	FItem := Value;
end;
...
function TDynamicPropertyInspector.GetProperties(
	inComponent: TPersistent): TLrDynamicProperties;
var
	ppi: PPropInfo;
	obj: TObject;
begin
	...
	begin
		...
		if (obj <> nil) and (obj is TLrDynamicProperties) then
			Result := TLrDynamicProperties(obj)
	end;
end;

function TDynamicPropertyInspector.CreateProperty(
	inType: TLrDynamicPropertyType): TDynamicProperty;
const
	cPropClasses: array[ptString..ptEvent] of TClass = ( TDynamicProperty,
		TScriptEventProperty );
begin
