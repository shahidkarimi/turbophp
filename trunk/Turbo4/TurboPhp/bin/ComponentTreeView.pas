...
type
	TComponentTreeFilterEvent = function(
		inObject: TObject): Boolean of object;
	//
	...
	protected
		...
		procedure AddItems(inNode: TTreeNode; inContainer: TWinControl);
...
procedure TComponentTreeForm.SetRoot(const Value: TWinControl);
begin
	if (Value <> FRoot) then
	begin
		if FRoot <> nil then
...
begin
	...
	for i := 0 to Pred(inContainer.ControlCount) do
		AddItem(inNode, inContainer.Controls[i]);
end;
